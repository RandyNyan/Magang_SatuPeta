...
<div class="space-y-6 max-w-2xl mx-auto bg-white p-6 rounded-xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
    ...
    <div>
        <h2 class="text-xl font-bold text-gray-800 tracking-tight">Tulis Berita Baru</h2>
        <p class="text-xs text-gray-500">Terbitkan berita atau pengumuman resmi ke portal geospasial.</p>
    </div>

    <form action="{{ route('manajemen.konten.berita.store') }}" method="POST" class="space-y-4">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="md:col-span-2">
                ...
                <input type="text" name="judul" id="judul" required placeholder="Contoh: Launching Fitur Baru Smart Facility Map" value="{{ old('judul') }}"
            ...
            <div>
                ...
                <input type="text" name="penulis" id="penulis" placeholder="Kosongkan untuk Admin" value="{{ old('penulis') }}"
                       class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
                @error('penulis')
                    <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
                @enderror
        ...
        <div>
            ...
            <textarea name="ringkasan" id="ringkasan" rows="2" placeholder="Masukkan ringkasan singkat artikel berita..."
                      class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">{{ old('ringkasan') }}</textarea>
            @error('ringkasan')
                <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="konten" id="konten" rows="8" required placeholder="Tuliskan isi berita lengkap di sini..."
                      class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">{{ old('konten') }}</textarea>
            @error('konten')
                <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
            @enderror
        </div>
