...
<div class="space-y-6 max-w-7xl mx-auto">
    ...
    <div>
        <h2 class="text-xl font-bold text-gray-800 tracking-tight">Tambah Peta Baru</h2>
        <p class="text-xs text-gray-500">Isi formulir berikut untuk menghubungkan data spasial dari PostgreSQL/PostGIS atau WMS Server.</p>
    </div>

    <form action="{{ route('manajemen.peta.store') }}" method="POST" id="peta-form">
        @csrf
            ...
            <div class="lg:col-span-6 bg-white p-6 rounded-xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="judul_mapset" id="judul_mapset" required placeholder="Contoh: Kepadatan Penduduk Sidoarjo" value="{{ old('judul_mapset') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="skala" id="skala" required placeholder="Contoh: 1:50.000" value="{{ old('skala') }}"
                               class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
                        @error('skala')
                            <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="tahun_data" id="tahun_data" required placeholder="2026" value="{{ old('tahun_data', date('Y')) }}"
                               class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="tingkat_penyajian" id="tingkat_penyajian" required
                                class="w-full text-xs px-3 py-2 border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
                            <option value="Desa/Kelurahan" {{ old('tingkat_penyajian') === 'Desa/Kelurahan' ? 'selected' : '' }}>Desa/Kelurahan</option>
                            <option value="Kecamatan" {{ old('tingkat_penyajian') === 'Kecamatan' ? 'selected' : '' }}>Kecamatan</option>
                            <option value="Kabupaten/Kota" {{ old('tingkat_penyajian') === 'Kabupaten/Kota' ? 'selected' : '' }}>Kabupaten/Kota</option>
                            <option value="Provinsi" {{ old('tingkat_penyajian') === 'Provinsi' ? 'selected' : '' }}>Provinsi</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="cakupan_wilayah" id="cakupan_wilayah" required
                                class="w-full text-xs px-3 py-2 border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
                            <option value="Desa/Kelurahan" {{ old('cakupan_wilayah') === 'Desa/Kelurahan' ? 'selected' : '' }}>Desa/Kelurahan</option>
                            <option value="Kecamatan" {{ old('cakupan_wilayah') === 'Kecamatan' ? 'selected' : '' }}>Kecamatan</option>
                            <option value="Kabupaten/Kota" {{ old('cakupan_wilayah') === 'Kabupaten/Kota' ? 'selected' : '' }}>Kabupaten/Kota</option>
                            <option value="Provinsi" {{ old('cakupan_wilayah') === 'Provinsi' ? 'selected' : '' }}>Provinsi</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-3 pt-3 border-t border-gray-50">
                    ...
                    <div class="flex space-x-6">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="sumber_peta" value="wms" class="form-radio text-blue-600 focus:ring-blue-400" {{ old('sumber_peta', 'wms') === 'wms' ? 'checked' : '' }}>
                            <span class="ml-2 text-xs font-semibold text-gray-600">WMS (GeoServer URL)</span>
                        </label>
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="sumber_peta" value="pgsql" class="form-radio text-blue-600 focus:ring-blue-400" {{ old('sumber_peta') === 'pgsql' ? 'checked' : '' }}>
                ...
                <div id="wms-group" class="p-4 bg-gray-50 border border-gray-100 rounded-xl space-y-3">
                    <div>
                        ...
                        <input type="url" name="link_openlayer" id="link_openlayer" placeholder="https://geoserver.example.com/geoserver/wms?layers=..." value="{{ old('link_openlayer') }}"
                               class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
                    </div>
                </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="open_layer_id" id="open_layer_id"
                            ...
                            @foreach($openLayers as $ol)
                                <option value="{{ $ol->id }}" data-table="{{ $ol->pgsql_table }}" {{ old('open_layer_id') == $ol->id ? 'selected' : '' }}>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        selTable.addEventListener('change', function() {
            ...
            fetch(`/api/spatial/columns/${table}`)
                ...
                .then(cols => {
                    ...
                    cols.forEach(col => {
                        // only display non-geometry columns
        ...
        selColumn.addEventListener('change', function() {
            if (this.value) {
                ...
                // If container is empty, add default ranges
        ...
        // Hex to Rgba helper for opacity styling in OL
        ...
        // OpenLayers Styling function based on rule builder
        const styleCache = {};
        ...
        function updateStylesAndSLD() {
            ...
            // Clear style cache and trigger map redraw
