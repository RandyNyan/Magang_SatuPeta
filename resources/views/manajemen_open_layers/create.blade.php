...
<div class="space-y-6">
    ...
    <div>
        <h2 class="text-xl font-bold text-gray-800 tracking-tight">Buat Open Layer Spasial</h2>
        <p class="text-xs text-gray-500">Gunakan toolbar gambar pada peta di sebelah kanan untuk menambahkan titik, garis, atau bidang.</p>
    </div>

    <form action="{{ route('manajemen.open-layers.store') }}" method="POST" id="layerForm" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
        ...
        <div class="bg-white p-5 rounded-xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-4">
            <div>
                ...
                <input type="text" name="nama_layer" id="nama_layer" required placeholder="Contoh: Titik Puskesmas Surabaya"
            ...
            <div>
                ...
                <select name="tipe_layer" id="tipe_layer" required
                    ...
                    <option value="Polygon">Polygon (Area/Bidang)</option>
                    <option value="Point">Point (Titik Koordinat)</option>
                    <option value="Line">Line (Garis/Jaringan)</option>
                </select>
                @error('tipe_layer')
                    <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3" placeholder="Tuliskan keterangan mengenai data spasial ini..."
                          class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400"></textarea>
                @error('deskripsi')
                    <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
                @enderror
            </div>
...
<script>
    ...
    tipeLayerSelect.addEventListener('change', function() {
        // Confirm before switching if they already drew shapes
    ...
    // Save previous val to revert if cancel
    tipeLayerSelect.dataset.lastVal = tipeLayerSelect.value;
    updateDrawingControls();
    ...
    map.on('pm:create', function(e) {
        ...
        // Open a small prompt for feature properties/attributes
    ...
    document.getElementById('layerForm').addEventListener('submit', function(e) {
        ...
        if (data.features.length === 0) {
            e.preventDefault();
            alert('Silakan gambar minimal satu objek pada peta (gunakan toolbar Leaflet-Geoman di sebelah kiri atas peta).');
        }
    });
</script>
@endpush
