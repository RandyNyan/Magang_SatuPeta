...
<div class="space-y-6 max-w-lg mx-auto bg-white p-6 rounded-xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
    ...
    <div>
        <h2 class="text-xl font-bold text-gray-800 tracking-tight">Edit Organisasi</h2>
        <p class="text-xs text-gray-500">Sesuaikan informasi detail Organisasi Perangkat Daerah.</p>
    </div>

    <form action="{{ route('manajemen.konten.organisasi.update', $organisasi->id) }}" method="POST" class="space-y-4">
        @csrf

        <div>
            <label for="nama" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Nama Organisasi / OPD</label>
            <input type="text" name="nama" id="nama" required value="{{ old('nama', $organisasi->nama) }}"
        ...
        <div>
            ...
            <textarea name="deskripsi" id="deskripsi" rows="3"
                      class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">{{ old('deskripsi', $organisasi->deskripsi) }}</textarea>
            @error('deskripsi')
                <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" name="alamat" id="alamat" value="{{ old('alamat', $organisasi->alamat) }}"
                   class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
            @error('alamat')
                <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
            @enderror
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="telepon" id="telepon" value="{{ old('telepon', $organisasi->telepon) }}"
                       class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
                @error('telepon')
                    <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $organisasi->email) }}"
                       class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
                @error('email')
                    <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
                @enderror
        ...
        <div>
            ...
            <input type="url" name="website" id="website" value="{{ old('website', $organisasi->website) }}"
                   class="w-full text-xs px-3.5 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400">
            @error('website')
                <span class="text-[10px] text-red-500 mt-1 block">{{ $message }}</span>
            @enderror
